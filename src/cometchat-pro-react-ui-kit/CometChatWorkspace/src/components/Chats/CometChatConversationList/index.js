...
class CometChatConversationList extends React.Component {
    ...
    render() {
        ...
        const conversationList = this.state.conversationlist.map((conversation, key) => {
            ...
            return <CometChatConversationListItem key={conversation.conversationId} conversation={conversation}
                                                  selectedConversation={this.selectedConversation}
                                                  loggedInUser={this.loggedInUser} handleClick={this.handleClick}
        ...
        let closeBtn = <div css={chatsHeaderCloseStyle(navigateIcon, theme)} className="header__close"
        ...
        const chatList = (
            <div css={chatsWrapperStyle(this.props, theme)} className="chats">
                <div css={chatsHeaderStyle(theme)} className="chats__header">
                    {closeBtn}
                    <h4 css={chatsHeaderTitleStyle(this.props)} className="header__title"
                        dir={Translator.getDirection(this.props.lang)}>
                        {Translator.translate("CHATS", this.props.lang)}
                    </h4>
                </div>
                {messageContainer}
                <div css={chatsListStyle()} className="chats__list" onScroll={this.handleScroll}
        ...
        if (this.props._parent === "") {
            chatListWrapper = <CometChatContextProvider
                ref={el => (this.contextProviderRef = el)}>{chatList}</CometChatContextProvider>;
        }
