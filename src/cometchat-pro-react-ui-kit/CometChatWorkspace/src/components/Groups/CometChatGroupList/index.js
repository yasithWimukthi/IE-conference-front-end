...
class CometChatGroupList extends React.PureComponent {
    ...
    render() {
        ...
        const groups = this.state.grouplist.map(group => {
            ...
            return <CometChatGroupListItem key={group.guid} group={group} selectedGroup={selectedGroup}
                                           clickHandler={this.handleClick}/>;
        });

        let createGroupBtn = (
            <div css={groupAddStyle(addIcon, theme)} title={Translator.translate("CREATE_GROUP", this.props.lang)}
        ...
        let closeBtn = <div css={groupHeaderCloseStyle(navigateIcon, theme)} className="header__close"
        ...
        if (this.state.enableSearchGroup) {
            searchGroup = (
                <div css={groupSearchStyle()} className="groups__search">
                    ...
                    <input type="text" autoComplete="off" css={groupSearchInputStyle()} className="search__input"
                           placeholder={Translator.translate("SEARCH", this.props.lang)} onChange={this.searchGroup}/>
                </div>
            );
        }
        ...
        if (this.state.createGroup) {
            createGroup = <CometChatCreateGroup theme={this.props.theme} close={() => this.createGroupHandler(false)}
            ...
            <React.Fragment>
                <div css={groupWrapperStyle(this.props, theme)} className="groups">
                    <div css={groupHeaderStyle(theme)} className="groups__header">
                        {closeBtn}
                        <h4 css={groupHeaderTitleStyle(this.props)} className="header__title"
                    ...
                    {searchGroup}
                    {messageContainer}
                    <div css={groupListStyle()} className="groups__list" onScroll={this.handleScroll}
        ...
        if (this.props._parent === "") {
            groupListWrapper = <CometChatContextProvider
                ref={el => (this.contextProviderRef = el)}>{groupListTemplate}</CometChatContextProvider>;
        }
